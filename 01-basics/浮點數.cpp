...
//long long my_long;: 要求 long 是 64 位元整數。在不同的系統上，long 的大小可能不同（有時是 32 位元）。為了確保能穩定地處理像 12345678912345 這樣大的數字，使用 long long 是最保險、最標準的做法。

//scanf(...): 將所有格式指定符放在一個字串中，並用空格隔開，scanf 會自動根據空格或換行來切分輸入的資料，並依次存入後面用 & (取址運算子) 指定的變數記憶體位置中。
