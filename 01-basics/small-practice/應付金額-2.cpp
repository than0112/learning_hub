...
/*核心觀念與程式碼解說
這個演算法的核心是一種「貪婪演算法 (Greedy Algorithm。永遠先用最大面額的錢去找零，找完後，再用剩下的錢去處理次大面額的，依此類推，直到找完為止。
...
將 change 變數的值更新為這個餘數，然後帶著這個新的、較小的金額，去進行下一個面額的計算。

這個「除法取商數，模數取餘數更新」的模式會一直重複，直到最後處理完 1 元銅板為止
*/