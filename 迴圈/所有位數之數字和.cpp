...
#include <iostream>
#include <cmath>   // 為了使用 abs() 函式來處理負數
#include <limits> // 為了輸入驗證
...
while 迴圈: 我們將這兩個步驟放在一個 while (temp_number > 0) 迴圈中，這樣程式就會不斷地重複「取得個位數並加總 -> 移除個位數」，直到 temp_number 變成 0 為止，此時所有的位數都已經被處理完畢。
...
abs(temp_number): 使用 <cmath> 函式庫中的 abs() 函式。這一步確保了即使輸入是負數（例如 -123），程式也能正確計算出其位數和（1 + 2 + 3 = 6），而不是產生非預期的結果。
*/