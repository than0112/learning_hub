...
generator：就是我們根據這個演算法建立出來的一個「產生器」物件。
...
uniform_int_distribution<int>(1, max_number)：代表我們想要一個「整數」的「均勻分佈」，且範圍落在 1 到 max_number 之間（包含兩端）。「均勻分佈」保證了這個範圍內的每個數字被抽中的機率都是相等的。
